...
[ -z "${MAVEN_USER_TEST_NEW_CREDS+x}" ] && \
  fail "Set MAVEN_USER to the Sonatype OSSRH user"
[ -z "${MAVEN_PASSWORD_TEST_NEW_CREDS+x}" ] && \
...
bazel run --define "maven_repo=${MAVEN_REPO}" --define "maven_user=${MAVEN_USER_TEST_NEW_CREDS}" \
  --define "maven_password=${MAVEN_PASSWORD_TEST_NEW_CREDS}" --define gpg_sign=true \
