...
public final class FuzzTargetRunner {
  ...
  private static final Set<Long> ignoredTokens =
      Opt.ignore.stream().map(Long::parseUnsignedLong).collect(Collectors.toSet());
  ...
  /**
   * Immediately terminates the process without performing any cleanup. Neither JVM shutdown hooks
   * nor native exit handlers are called.
   *
