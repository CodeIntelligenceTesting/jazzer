 ...
 *
 * This class has to be loaded by the bootstrap class loader since the native library it loads links in libFuzzer and
 * the Java hooks, which have to be on the bootstrap path so that they are seen by Java standard library classes, need
 * to be able to call native libFuzzer callbacks.
 */
