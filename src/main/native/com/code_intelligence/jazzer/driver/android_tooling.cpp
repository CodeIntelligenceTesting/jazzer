...
std::string getNativeString(JNIEnv* env, jbyteArray javaString) {
    ...
    if (!elements) {
        std::cout << "Passed a null string to the JNI.\n";
        exit(1);
    }
...
[[maybe_unused]] jint
Java_com_code_1intelligence_jazzer_android_AndroidRuntime_registerNatives(JNIEnv* env, jobject obj, jbyteArray runtimeLibs) {
    ...
    if (strcmp(USE_EMBEDDED, nativeString.c_str()) == 0) {
        std::cout << "Embedded native libraries will be used to initialize ART.\n";
        setenv("LD_LIBRARY_PATH", EMBEDDED_DIR, true);
        std::cout << "LD_LIBRARY_PATH = " << getenv("LD_LIBRARY_PATH") << std::endl; 
    } else {
        std::cout << "No runtime_libs defined, fuzzer dependencies may not be hermetic.";
    }
    ...
    if (handle == nullptr) {
        std::cout << "Unable to locate runtime libs. Check LD_LIBRARY_PATH.";
        exit(1);
    }
    // reset errors
    dlerror();
