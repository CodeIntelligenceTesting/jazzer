...
class FuzzTestExecutor {
  ...
  private final Map<String, Object> fuzzerArgs;
  ...
  private FuzzTestExecutor(Map<String, Object> fuzzerArgs) {
    this.libFuzzerArgs = null;
    this.fuzzerArgs = fuzzerArgs;
    this.javaSeedsDir = (Optional<Path>) fuzzerArgs.get("seedsDir");
  ...
  public static FuzzTestExecutor prepareUnchained(
    ...
    List<String> originalFuzzerArgs = Utils.getLibFuzzerArgs(context);
    Map<String, Object> fuzzerArgs = new HashMap<>();
    ...
    if (!corpusFilesOrDirs.isEmpty()
      ...
      javaSeedsDir =
          Optional.of(
              addInputAndSeedDirs(context, corpusFilesOrDirs, createDefaultGeneratedCorpusDir));
    }

    fuzzerArgs.put("seedsDir", javaSeedsDir);
    System.err.println("--------------------------   Java seeds dir: " + javaSeedsDir);

    dictionaryPath.ifPresent(s -> fuzzerArgs.put("dictionary", s));
    System.err.println("--------------------------   Corpus dirs/files: " + corpusFilesOrDirs);
    ...
    if (Utils.permissivelyParseBoolean(
      ...
      fuzzerArgs.put("useValueProfile", true);
    }

    translateJUnitTimeoutToSeconds(context)
        .ifPresent(seconds -> fuzzerArgs.put("timeout", seconds));

    return new FuzzTestExecutor(fuzzerArgs);
  }
  ...
  public static FuzzTestExecutor prepare(
    ...
    //    // Non-empty as it always contains FuzzingArgumentsProvider.
    //    ArgumentsSource lastSource = allSources.get(allSources.size() - 1);
    //    // Ensure that our ArgumentsProviders run last so that we can record all the seeds
    // generated by
    //    // user-provided ones.
    //    if (lastSource.value().getPackage() != FuzzTestExecutor.class.getPackage()) {
    //      throw new FuzzTestConfigurationError(
    ...
    //    String argv0 = originalLibFuzzerArgs.isEmpty() ? "fake_argv0" :
    // originalLibFuzzerArgs.remove(0);
    //
    ...
    //
    //    // When reproducing individual inputs, we must not add any corpus directories to the
    // command
    //    // line or libFuzzer will fail with "Not a directory: ...; exiting".
    //    Optional<Path> javaSeedsDir;
    //    if (!corpusFilesOrDirs.isEmpty()
    ...
    //      javaSeedsDir = Optional.empty();
    //    } else {
    //      // Only create the default generated corpus directory if it is used as the generated
    // corpus
    //      // directory, i.e., if the user didn't provide any custom corpus directories that come
    // before
    //      // it on the libFuzzer command line.
    //      boolean createDefaultGeneratedCorpusDir = corpusFilesOrDirs.isEmpty();
    //      javaSeedsDir =
    //          Optional.of(addInputAndSeedDirs(context, libFuzzerArgs,
    ...
    //    }
    //    // Disable libFuzzer's out of memory detection: It is only useful for native library
    // fuzzing,
    //    // which we don't support without our native driver, and leads to false positives where it
    // picks
  ...
  private static Optional<Long> translateJUnitTimeoutToSeconds(ExtensionContext context) {
    return Stream.<Supplier<Optional<Long>>>of(
            () ->
                AnnotationSupport.findAnnotation(context.getRequiredTestMethod(), Timeout.class)
                    .map(timeout -> timeout.unit().toSeconds(timeout.value())),
